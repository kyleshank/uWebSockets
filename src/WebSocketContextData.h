...
public:
    ...
    void publish(std::string_view topic, std::string_view message, OpCode opCode, bool compress, Subscriber* subscriber) {
        ...
        if (compression == DISABLED) {
            ...
            topicTree.publish(topic, {std::string_view(dst, dst_length), {}}, subscriber);
        } else {
            if (compress) {
                ...
                if (compression == SHARED_COMPRESSOR) {
                    ...
                    topicTree.publish(topic, {std::string_view(dst, dst_length), dst_compressed_length >= dst_length ? std::string_view(dst, dst_length) : std::string_view(dst_compressed, dst_compressed_length)}, subscriber);
                ...
                topicTree.publish(topic, {std::string_view(dst, dst_length), std::string_view(dst, dst_length)}, subscriber);
            }
        }
